pipeline {
    ...
    stages{
       ...
       stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh ('curl -v -k --user Admin:${JENKINS_API_TOKEN} -H "cache-control: no-cache" -H "content-type: application/x-www-form-urlencoded" --data "IMAGE_TAG=${IMAGE_TAG}" "http://192.168.52.139:8080//job/git-ops/buildWithParameters?token=gitops-token"')
                }
            }
       }
